...
- `server/modules/providers/services/provider-capabilities.service.ts` —— 类型和四个 provider 对象各追加一个 `supportsFork` 字段，保持在最后，跟条目 22 对 `supportsCompact` 的要求一样。#23 落地时 `opencode` 就地翻成 `true`。
