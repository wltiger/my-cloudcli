...
- `server/shared/types.ts` —— `NormalizedMessage` 追加了一个可选字段 `anchor`，放在索引签名之前的最后一个。冲突解决时不要把顺序打乱。
...
- `server/modules/providers/services/provider-capabilities.service.ts` —— 类型和四个 provider 对象各追加一个 `supportsFork` 字段，保持在最后，跟条目 22 对 `supportsCompact` 的要求一样。`opencode` 随 #23 已经就地翻成 `true`——同一个字段、同一个位置。`supportsClear` 随 #24 用同样方式追加在它后面。
...
- `src/components/chat/view/subcomponents/MessageComponent.tsx` —— 一个 import、`MessageComponentProps` 上一个 prop 及其解构、一个 `forkAnchor` 常量，以及两处 JSX：一处在用户气泡底部那一行、`MessageCopyControl` 之前，一处在 assistant 共用底栏、`MessageFullscreenControl` 之后。assistant 那处用的是它自己的判断条件（`shouldShowAssistantCopyControl && !message.isToolUse`），没有直接复用复制按钮那个标志——不要"顺手"合并掉。它也是刻意**不**出现在工具卡片、思考块和纯附件的用户分支上的。
- `src/components/chat/view/subcomponents/ChatMessagesPane.tsx` —— 声明、解构、并且只在顶层那个 `MessageComponent` 调用处往下传一个 prop。`ToolGroupContainer.tsx` 是**刻意**没有接的，这正是 Fork 不出现在工具卡片上的原因；将来想让它出现在那儿，那是一个新决定，不是漏了一行。
...
- `src/components/chat/hooks/useChatComposerState.ts` —— 一个 import、参数类型上两个字段及其解构、一个依赖项，以及 `executeCommand` 最上面一个提前 return 的分支。那个分支就是全部的拦截逻辑：不要把它挪到 `try` 里面，也不要把 `'clear'` 加进下面那个 skill/compact 的放行判断——那个放行的意思是“原样送给 provider”，跟 Clear 需要的正好相反。
...
- `src/i18n/locales/{en,zh-CN}/chat.json` —— 在 `compact` 之后插入一个新的顶层 `fork` 组，两个文件里 key 顺序一致。政策同条目 3：冲突时两边的 key 都留下；其他语种未动，会回落到英文。
