...
**同步官方时怎么办：** 保留我的。官方既没有 Fork 也没有 Anchor，每条消息上除了复制/朗读/全屏也没有别的操作，所以这里绝大部分是新文件里的全新代码，根本不会冲突。在共享的那几个分发点上，**照着形状重新加一遍，不要套用原始 diff**——每个点都只加了一处：
...
- `server/modules/providers/services/provider-capabilities.service.ts` —— 类型和四个 provider 对象各追加一个 `supportsFork` 字段，保持在最后，跟条目 22 对 `supportsCompact` 的要求一样。`opencode` 随 #23 已经就地翻成 `true`——同一个字段、同一个位置。
...
- `src/utils/api.js` —— 两个方法，直接追加在 `renameSession` 后面。
...
- `src/components/chat/view/ChatInterface.tsx` —— 两个 import、解构出一个能力标志、紧跟在 `handleSessionEstablished` 之后的 `handleSessionForked` 和 `useSessionFork` 调用（它依赖前者）、`<ChatMessagesPane>` 上一个 prop，以及作为 `CommandResultModal` 兄弟节点追加的 `<SessionForkDialog>`——没有包裹任何已有节点，所以现有 JSX 一行缩进都没动。
- `src/components/chat/hooks/useChatProviderState.ts`、`useChatMessages.ts`、`src/components/chat/types/types.ts`、`src/stores/useSessionStore.ts` —— 各自一行或一小块，全都是照着紧挨在它们上面的那几个 compact 字段写的。
