...
## 17. Codex model catalog fetched live instead of a stale snapshot
...
**On upstream conflict:** This is a plain gap-fill rather than a fork preference — worth proposing upstream, and drop this entry if they ship an equivalent, ideally through a real SDK method rather than shelling out to an undocumented `debug` subcommand (this fork's known risk: it could be renamed or removed without notice in a future `@openai/codex` release). Until then keep mine. If upstream touches `codex-models.provider.ts` for something else (e.g. their own SDK version bump), reapply the `fetchLiveCodexModels()` subprocess path and keep `codex` in `UNCACHED_PROVIDERS`.

## 18. Claude model catalog fetched live via SDK instead of a hardcoded list
...
**On upstream conflict:** Also a plain fix reviving previously-written-but-disabled code, not a fork preference — worth proposing upstream, and drop this if they ship a fix. Until then keep mine. `CLAUDE_FALLBACK_MODELS` stays as the last-resort fallback for unauthenticated/errored calls; if upstream changes its contents that's independent of the live-fetch path and merges normally.
