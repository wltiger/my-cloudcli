...
## 16. Push notifications collapse to one-per-session, cleared on open

**Files:** `server/modules/notifications/services/notification-orchestrator.service.js`, `public/sw.js`, `src/components/app/AppContent.tsx`

**Why:** Upstream tags each push `provider:sessionId:code`, so a session that fires a permission request, then a stop, then an error keeps three separate notifications stacked in the phone's notification center forever — none of them ever gets cleared, including the ones you already acted on inside the app. This fork drops `code` from the tag when a `sessionId` is present (`provider:sessionId`), so the OS's native same-tag replacement (`renotify: true` was already set) collapses a session down to its single latest notification as new pushes arrive. Opening that session in the app additionally posts `{ type: 'CLEAR_SESSION_NOTIFICATIONS', sessionId }` to the service worker, which closes any already-shown notification whose `data.sessionId` matches — a safety net for the case where the tag-replacement path was bypassed (e.g. the notification arrived while the SW was asleep and the tag scheme changed mid-flight). Sessionless events (e.g. `push.enabled`) keep a `provider:global:code` tag so they don't collapse into each other.

**On upstream conflict:** Keep mine. Small, self-contained footprint: one tag expression in the orchestrator, one new `message` branch in `sw.js` (alongside the existing `REFRESH_CACHE` handler), one `useEffect` in `AppContent.tsx` keyed on the route's `sessionId`. If upstream reworks the push payload/tag scheme, keep the "one notification per session, cleared on open" behavior and re-derive the tag/clear logic against their new shape rather than dropping the feature.
