...
import { directUrl } from '../voiceApi';

import { readVoiceStreamConfig } from './config';
import { PcmStreamPlayer } from './pcmPlayer';

// Streaming counterpart to lib/voicePlayer: same subscribe/getSnapshot/
...
class StreamingVoicePlayer {
  ...
  private async play(id: string, content: string) {
    ...
    try {
      const response = await fetch(directUrl(config.baseUrl.trim(), '/audio/speech'), {
        method: 'POST',
        headers: {
