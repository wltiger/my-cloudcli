...
test('an edit that shortens the transcript is caught by total, not by overlap', () => {
  // What the reader had open: rows 21..60 of a 60-message session.
  const cached = range(21, 60);
  // The edit dropped 41..60 and the replacement turn landed, so the newest page
  // is rows 23..40 of the surviving conversation plus that new turn.
  const latest = [...range(23, 40), message(101), message(102)];

  // The stitch this module normally relies on has nothing to anchor on: the
  // cached tail it would match against is exactly what the edit deleted.
