...
import { useChatWidthClasses } from '@/shared/hooks/useChatTypography';
...
type ChatComposerProps = {
  ...
  /**
   * Whether sending that replacement also puts tracked files back. Settled by
   * the provider, never a choice: OpenCode replaces a message by reverting its
   * session, which rolls the working tree back too, while Claude and Codex only
   * re-run the conversation. The notice says one or the other, and saying the
   * wrong one is a promise about the reader's files that does not hold.
   */
  editRestoresFiles: boolean;
        ...
        <div className="mx-auto mb-2 flex max-w-[54.25rem] items-center gap-2 rounded-xl border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-foreground">
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="text-muted-foreground">
              {editRestoresFiles
                ? t('composer.editing.filesReverted')
                : t('composer.editing.filesNotReverted')}
            </span>
          </span>
          <button
            type="button"
