...
import MessageFullscreenControl from '@/modules/chat/transcript/MessageFullscreenControl';
...
const MessageComponent = memo(({ message, prevMessage, createDiff, onFileOpen, showRawParameters, showThinking, selectedProject, provider, onEditMessage, onForkFromMessage }: MessageComponentProps) => {
  ...
  // Whichever Anchor field the message carries is enough to fork at it, and
  // which one it is decides which backend does it — see `useSessionFork`.
  // Neither is present on anything a fork cannot start from: live-streamed
  // turns, and history older than the last compaction.
  const canForkFromMessage = Boolean(message.transcriptAnchorId || message.anchor);
            ...
            {userCopyContent.trim().length > 0 || (!message.images?.length && !message.files?.length) ? (
              <div className="group max-w-full rounded-2xl rounded-br-md border border-border/60 bg-muted/60 px-3 py-2 text-foreground shadow-sm dark:bg-gray-800/60 sm:px-4">
                ...
                <div className="mt-1 flex items-center justify-end gap-1 text-xs text-muted-foreground">
                  ...
                  {onForkFromMessage && canForkFromMessage && (
                    <button
                      type="button"
