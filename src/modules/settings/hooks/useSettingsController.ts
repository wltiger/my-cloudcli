...
import { useChatFontLevel, useChatWidthLevel } from '@/shared/hooks/useChatTypography';
...
export function useSettingsController({ isOpen, initialTab }: UseSettingsControllerArgs) {
  ...
  const saveSettings = useCallback(async () => {
    ...
    try {
      ...
      // merge-patch rather than four racing requests. This is also what retired
      // the fork's read-merge-write of `claude-settings`: each preference is now
      // its own key, so saving here can no longer drop a key it does not own.
      writeUserPreferences({
        claudePermissions: {
