...
export const api = {
  ...
  // Fork (this fork's own, Anchor-based flavor): the pre-filled dialog name,
  // then the fork itself at a message's Anchor. Deliberately named apart from
  // `forkSession` above, which is upstream's own transcriptAnchorId flavor --
  // the two mechanisms are kept side by side rather than merged.
  forkSessionName: (sessionId: string) =>
    get(`/api/providers/sessions/${encodeURIComponent(sessionId)}/fork-name`),
  forkSessionAtAnchor: (sessionId: string, anchor: string, name: string) =>
    post(`/api/providers/sessions/${encodeURIComponent(sessionId)}/fork-at-anchor`, { anchor, name }),
  // Clear: retires the open conversation and opens an empty one beside it.
  clearSession: (sessionId: string) =>
    post(`/api/providers/sessions/${encodeURIComponent(sessionId)}/clear`),
...
/**
 * Builds a URL against the user's own OpenAI-compatible voice endpoint, used by the
 * voice helpers below (and by the chat module's streaming/voice-list callers) to bypass
 * the CloudCLI proxy when a base URL is configured.
 */
export function voiceDirectUrl(baseUrl: string, path: string): string {
