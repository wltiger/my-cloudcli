...
export type NormalizedMessage = {
  ...
  /**
   * The Anchor this message can be Forked at — the transcript row the fork is
   * taken at, named the way this message's provider names it. Computed
   * backend-side from the raw transcript rows, because one agent turn spans
   * several rows and the ids on rendered messages are CloudCLI's own. Absent
   * when a fork cannot be started from this message at all (live events, and
   * anything before the last compaction boundary), in which case the frontend
   * simply offers no entry.
   */
  anchor?: string;
  [key: string]: unknown;
};
